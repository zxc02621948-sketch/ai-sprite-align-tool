...
過去這個工具只能對齊特效（爆炸、斬擊那種逐格素材），現在加入了「人物行走圖」模式。AI 生成的角色行走圖每一格位置常常不一致，丟進來選 `3x4 行走`、按一下自動對齊，就能順順走。

![人物行走圖自動對齊](demo/walk-align-demo.gif)

過程中也處理了 AI 行走圖一個常見的麻煩：角色的頭髮會飄動，用整體輪廓中心對齊時，飄動的頭髮會把整個身體往旁邊拉，播放起來整個人左右晃。工具改成偵測「身體核心」來對齊、忽略飄動的頭髮——身體站穩，頭髮照樣自然甩。
